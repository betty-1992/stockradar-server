/**
 * StockRadar Server — Railway/로컬 배포용
 * Railway 환경변수: FMP_KEY, PORT(자동)
 * 로컬: FMP_KEY=xxxxx node server.js
 */
'use strict';
const http=require('http'),https=require('https'),fs=require('fs'),path=require('path'),url=require('url');
const PORT=process.env.PORT||3001;
let FMP_KEY=process.env.FMP_KEY||'';
...
// ── HTTP GET ─────────────────────────────────────────────
function httpGet(reqUrl,hdrs={}){return new Promise((res,rej)=>{
  const u=new URL(reqUrl);
  const mod=u.protocol==='https:'?https:http;
  const req=mod.request({hostname:u.hostname,path:u.pathname+u.search,method:'GET',headers:{'User-Agent':'Mozilla/5.0','Accept':'application/json',...hdrs},timeout:20000},r=>{
    const ch=[];r.on('data',c=>ch.push(c));
    r.on('end',()=>{try{res(JSON.parse(Buffer.concat(ch).toString()));}catch(e){rej(new Error('JSON fail:'+reqUrl.slice(0,60)));}});
  });
...
async function fetchKrx(market='STK'){
  ...
  try{
    const data=await new Promise((res,rej)=>{
      const req=https.request({hostname:'www.krx.co.kr',path:'/comm/bldAttendant/getJsonData.cmd',method:'POST',
        headers:{'Content-Type':'application/x-www-form-urlencoded','Content-Length':Buffer.byteLength(body),'Referer':'https://www.krx.co.kr/','User-Agent':'Mozilla/5.0'},timeout:15000},r=>{
  ...
  for(let i=0;i<syms.length;i+=30){
    ...
    try{
      ...
      (d?.quoteResponse?.result||[]).forEach(q=>{
        const id=q.symbol.replace(/\.(KS|KQ)$/,'');
        const px=q.regularMarketPrice||0,h52=q.fiftyTwoWeekHigh||0,l52=q.fiftyTwoWeekLow||0;
        if(!px)return;
        res[id]={price:px,change:q.regularMarketChange||0,changeRate:q.regularMarketChangePercent||0,
          volume:q.regularMarketVolume||0,high52:h52,low52:l52,
          drop:h52>0?Math.max(0,Math.round(((px-h52)/h52)*-100)):0,
          pe:q.trailingPE||0,div:q.trailingAnnualDividendYield?+(q.trailingAnnualDividendYield*100).toFixed(2):0,
          mc:q.marketCap?Math.round(q.marketCap/1e6):0,eps:q.epsTrailingTwelveMonths||0,
          name:q.shortName||q.longName||id,currency:isKr?'KRW':'USD'};
      });
...
async function fmpScreener(key){
  ...
  for(const ex of['NASDAQ','NYSE','AMEX']){
    try{
      const d=await httpGet(`https://financialmodelingprep.com/api/v3/stock-screener?marketCapMoreThan=300000000&exchange=${ex}&limit=300&apikey=${key}`);
...
async function fmpFundamentals(syms,key){
  ...
  for(let i=0;i<list.length;i++){
    ...
    if(i%5===0){try{const gr=await httpGet(`https://financialmodelingprep.com/api/v3/income-statement-growth/${sym}?limit=1&apikey=${key}`);if(Array.isArray(gr)&&gr[0]){res[sym]=res[sym]||{};res[sym].rev=gr[0].growthRevenue?+(gr[0].growthRevenue*100).toFixed(1):0;}}catch(_){}await sleep(400);}
...
async function runBatch(key){
  ...
  p(14,`Yahoo 한국 ${krList.length}개 시세...`);
  ...
  p(34,'FMP 미국 스크리너...');const usRaw=await fmpScreener(key);p(50,`미국 ${usRaw.length}개`);
  ...
  p(52,`Yahoo 미국 ${usIds.length}개 시세...`);
  ...
  p(100,`완료! 총 ${list.length}개 (한국 ${list.filter(s=>/^\d{6}$/.test(s.id)).length} + 미국 ${list.filter(s=>!/^\d{6}$/.test(s.id)).length})`);
  return list;
}
...
const server=http.createServer(async(req,res)=>{
  ...
  if(pathname==='/'||pathname==='/index.html'){
    ...
    res.end(fs.existsSync(f)?fs.readFileSync(f):'<h2>StockRadar.html 파일이 없습니다</h2>');return;
  }

  res.setHeader('Content-Type','application/json;charset=utf-8');

  if(pathname==='/api/status'){res.writeHead(200);res.end(JSON.stringify({ok:true,stocks:cache.stocks?.length||0,lastBatch:cache.lastBatch,batchRunning,fmpKeySet:!!FMP_KEY}));return;}
  ...
  if(pathname==='/api/batch'){
    const key=query.fmp||FMP_KEY;
    if(!key){res.writeHead(400);res.end(JSON.stringify({error:'FMP 키 필요. ?fmp=KEY 또는 FMP_KEY 환경변수'}));return;}
    if(batchRunning){res.writeHead(409);res.end(JSON.stringify({error:'배치 이미 실행 중'}));return;}
    batchRunning=true;batchProg={pct:0,msg:'시작',log:[]};FMP_KEY=key;
  ...
  res.writeHead(404);res.end(JSON.stringify({error:'Not Found',routes:['GET /','GET /api/status','GET /api/stocks','GET /api/prices?symbols=','GET /api/krx?market=STK','POST /api/batch?fmp=KEY','GET /api/batch/status']}));
});

server.listen(PORT,()=>{console.log(`\nStockRadar Server → http://localhost:${PORT}\n종목: ${cache.stocks?.length||0}개 | FMP키: ${FMP_KEY?'✅':'❌'}\n`);});
