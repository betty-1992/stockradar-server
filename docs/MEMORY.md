...
- ✅ **KR 유니버스 23배 확장 완료 — 117 → 2,708** (실제 `server/data.db` 주입 완료):
  - KRX 공식 API 는 현재 IP 에서 "LOGOUT" / pykrx 도 같은 이유로 0개 반환 → **FinanceDataReader 로 우회 성공** (Naver 금융 등 폴백)
  - `server/scripts/data/kr-universe.csv` 2,770행 생성 (KOSPI 949 + KOSDAQ 1,821, market_cap 포함)
  - `fetch-krx-universe.js` 실행 결과: inserted 2,591 / skipped_curated 113 / failed 66 (우선주·신주인수권 등 6자리 아닌 심볼)
  - `fetch-kr-stocks.js` `loadTickers()` DB 우선 + `ORDER BY is_curated, market_cap DESC` — LIMIT 사용 시 실제 수집 대상(큐레이션 제외) 중 시총 큰 종목부터 커버. 현재 상위 5: 삼성전자우·SK스퀘어·HD현대중공업·HD현대일렉트릭·미래에셋증권
- ⏸️ **Yahoo 광범위 IP 차단 지속** — 일반 집 IP + 모바일 핫스팟 + 오늘 재연결한 새 IP(220.70.42.142) 모두 getcrumb 429. 브라우저 cookie 주입해도 quoteSummary 자체 서버 측 429. TLS fingerprint + HttpOnly cookie 누락 복합 원인 추정. **24~72h 대기 외 즉시 해결 방법 없음**

### 📋 내일 작업 순서
1. **Yahoo 차단 확인** (가장 먼저):
   ```
   curl -sS -o /dev/null -w "%{http_code}\n" \
     -A 'Mozilla/5.0' -b /tmp/yc.txt \
     'https://query1.finance.yahoo.com/v1/test/getcrumb'
   ```
   200 이면 아래 2번으로, 여전히 429 면 다른 IP(VPN/핫스팟) 시도 또는 추가 대기
2. **Phase 2 Yahoo 수집 실행** (차단 풀리면):
   - 점진 실행: `cd server && KR_LIMIT=100 node scripts/fetch-kr-stocks.js` — 시총 상위 100개부터
   - 성공 시 LIMIT 올려서 전체 2,591개 수집 (약 45분 예상, 1초 딜레이 기준)
   - US 는 기존 fetch-us-stocks 로 500개 (S&P500) 수집
3. **Phase 3 재배포** — seed 자동화 덕분에 안전. `/api/universe` DB 전환 재시도. 로컬에서 total 증가 확인 후 Railway 배포
4. **큐레이션 종목 market_cap 보강 (옵션)** — 현재 116개 큐레이션 종목은 market_cap NULL. KRX CSV 재활용해서 UPDATE 별도 스크립트 고려 (단 is_curated=1 보호 정책 예외 로직 필요)
5. **US 유니버스 확장 (옵션)** — fetch-us-stocks 현재 datahub S&P500(500). Russell 1000 / NASDAQ 상장사 등으로 확장 검토
...
- [x] **KR 유니버스 23배 확장 (117 → 2,708)** — 2026-04-20
  - 배경: 2026-04-18 "종목 수 800 → 더 많이" 확장 요구. KOREAN_TICKERS 하드코딩 116개 한계
  - 데이터 소스: **FinanceDataReader** (Naver 금융 등 다중 소스 폴백). pykrx 는 KRX 공식 API 의존이라 IP 차단 시 0개 반환 — 본 작업 진행 불가. FDR 은 통과
  - CSV: `server/scripts/data/kr-universe.csv` 2,770행 (KOSPI 949 + KOSDAQ 1,821 + market_cap)
  - 주입 결과: inserted 2,591 (source='krx', is_curated=0) / skipped_curated 113 (기존 큐레이션 보호) / failed 66 (우선주·신주인수권 등 6자리 아닌 심볼 — 무시 가능)
  - `fetch-kr-stocks.js` `loadTickers()`: DB 우선 + `ORDER BY is_curated, market_cap DESC` — LIMIT 사용 시 실제 수집 대상(큐레이션 제외) 중 시총 큰 종목부터 자동 커버
  - 재사용: CSV 갱신은 `python3 -c "import FinanceDataReader as fdr, csv; ..." > server/scripts/data/kr-universe.csv` 실행 후 `node scripts/fetch-krx-universe.js`
